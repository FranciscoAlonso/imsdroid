...
public class ScreenAV extends BaseScreen{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.e(TAG, String.format(">>>> OJO <<< SESSION ID: %d", NgnStringUtils.parseLong(super.mId, -1)));
		if(mAVSession == null){
		...
		mAVSession.incRef();
		Log.e(TAG, String.format("Context: %s",this));
	...
	private void loadTryingView(){
		...
		btPick.setOnClickListener(new View.OnClickListener() { //boton de atender
			@Override
			public void onClick(View v) {
				acceptCall();
			}
		});
		btHang.setOnClickListener(new View.OnClickListener() { //boton de colgar
			@Override
			public void onClick(View v) {
				hangUpCall();
			}
		...
		switch(mAVSession.getState()){
	        case INCOMING:
	        	mTvInfo.setText(getString(R.string.string_call_incoming)); //llamada entrante
	        	break;
	        case INPROGRESS:
	        case REMOTE_RINGING:
	        case EARLY_MEDIA:
	        default:
	        	mTvInfo.setText(getString(R.string.string_call_outgoing)); //llamada saliendo
	        	btPick.setVisibility(View.GONE); //se quita el boton de atender 
	        	break;
	    }
		...
		mMainLayout.removeAllViews(); // se limpia el layout
		mMainLayout.addView(mViewTrying); // se carga el view de trying, para entrante o saliente
		mCurrentView = ViewType.ViewTrying; // se cambia el tipo de view a trying
	}
	...
	private void loadVideoPreview(){
		...
		Log.e(TAG, String.format("Context2: %s",this));
	...
	private void startStopVideo(boolean bStart){
		...
		if(mViewLocalVideoPreview != null){
			...
			if(bStart){
				...
				Log.e(TAG, String.format("Context3: %s", getBaseContext().toString()));
